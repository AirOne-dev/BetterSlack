...
Slack's rail carries the same shapes inline. The `.icns` used to be a committed
binary with no recipe, so redrawing the mark left the app wearing the old one
and nothing said so; `scripts/build-icon.mjs` rasterises the ten sizes
`iconutil` wants, through `rsvg-convert`, ImageMagick or headless Chrome,
whichever is on the machine. It is not part of `pnpm check` -- an icon changes
when somebody redraws it.

The mark has four colours of its own rather than taking `currentColor`, so the
launcher no longer dims with the icons beside it on hover: there is no single
tint to dim. `LAUNCHER_CSS` does that with opacity instead, which is the only
