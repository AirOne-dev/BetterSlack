...
- **A tooltip's global listeners live only while it is showing.** `keydown`,
  ...
  It was reported as a mouse cursor flickering between arrow and pointer, and
  it was found by patching `addEventListener` in a live client and counting --
  not by reading, and not by watching for DOM churn, which showed nothing at all
  because there was none. One tooltip is visible at a time, so there is at most
  one set of these and usually none.
