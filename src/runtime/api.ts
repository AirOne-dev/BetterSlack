...
export interface ApiContext {
  /** Slack's own realtime events, shared by every mod in this client. */
  ...
  const api: PluginApi = {
    ...
    slack: (() => {
      ...
      return {
        ...
        /*
         * The client's one event surface, with every listener collected.
         *
         * Wrapped rather than handed over: a mod switched off must stop
         * hearing about messages, and its subscription is also what keeps the
         * loader's tap open. Left registered, a disabled mod would go on
         * receiving every message in the workspace for ever.
         */
        events: new Proxy(ctx.slackEvents, {
          get(target, key) {
            const value = Reflect.get(target, key) as unknown;
            if (typeof value !== 'function' || typeof key !== 'string' || !key.startsWith('on')) {
              return value;
            }
            return (...args: never[]) => {
              const cleanup = (value as (...a: never[]) => Cleanup).apply(target, args);
              cleanups.add(cleanup);
              return cleanup;
            };
          },
