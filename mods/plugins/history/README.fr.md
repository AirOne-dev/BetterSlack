...
Chaque carte peut être oubliée séparément, à côté du bouton qui vide tout. La page a une recherche qui porte sur tout ce qu'une ligne affiche — un nom se retrouve qu'il ait été la personne, le canal ou le mot changé —, des filtres par famille et cinq tris. Chaque ligne propose de copier l'ancien texte, et d'aller au message quand il en reste un. Et un message modifié répond de lui-même : le **(modifié)** de Slack devient le bouton, et cliquer dessus déplie sous le message ce qu'il disait avant — pas ce qu'il dit maintenant, qui est le message lui-même juste au-dessus. Le chevron et le dépliage bougent si Motion est installé, et ne bougent pas sinon. Slack marque déjà le message et pose déjà la marque là où la question se pose — il n'y répond simplement pas. Uniquement quand il y a quelque chose à montrer : Slack marque toutes les modifications, y compris celles faites avant l'installation, et ces étiquettes-là sont laissées exactement comme Slack les a dessinées.
