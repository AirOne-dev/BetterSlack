...
Each card can be forgotten on its own, beside the button that empties the lot. The page has a search that runs over everything a row draws — a name finds it whether it was the person, the channel, or the word that changed — filters by family, and five sorts. Each row offers the old text to copy, and takes you to the message where there is still one to land on. And an edited message answers for itself: Slack's own **(edited)** becomes the control, and clicking it unfolds every wording the message has had under the message, with the current one at full strength. Slack already marks the message and already puts the mark where the question is asked — it just does not answer it. Only where there is something to show: Slack marks every edit, including ones made before this was installed, and those labels are left exactly as Slack drew them.
