...
function statusFixture(v) {
  /*
   * A fixed hour rather than a moving one: the page is regenerated by a build
   * and a date computed from "now" would make every rebuild a diff.
   */
  const EXPIRES_AT = Date.UTC(2026, 7, 30, 15, 0) / 1000;
  const profile = {
    status_text: v.text ?? '',
